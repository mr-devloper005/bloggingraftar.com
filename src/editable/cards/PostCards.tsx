...
export function EditorialFeatureCard({ post, href, label = 'Featured read' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[420px] p-6 sm:p-8 lg:min-h-[500px]">
        ...
        <div className="relative z-10 flex h-full min-h-[360px] flex-col justify-end lg:min-h-[440px]">
          ...
          <h3 className="mt-5 max-w-3xl text-3xl font-black leading-tight tracking-[-0.04em] sm:text-4xl lg:text-5xl">{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className={`absolute left-3 top-3 rounded ${pal.darkBg} px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-white`}>No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-3 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 text-2xl font-black leading-tight tracking-[-0.03em] ${pal.panelText} sm:text-[1.7rem]`}>{post.title}</h2>
