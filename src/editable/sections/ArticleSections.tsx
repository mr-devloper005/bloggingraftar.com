  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-lg border ${pal.border} ${pal.darkBg} p-7 text-white shadow-[0_20px_60px_rgba(15,79,89,0.18)] sm:p-10 lg:p-12`}>
          <p className={`${dc.type.eyebrow} ${pal.accentSoftText}`}>{voice.eyebrow}</p>
          <h1 className="mt-5 max-w-4xl text-4xl font-black leading-tight tracking-[-0.04em] sm:text-5xl">{voice.headline}</h1>
          ...
          <form action={basePath} className="mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
            <select name="category" defaultValue={category || 'all'} className={`min-w-0 flex-1 rounded-lg border ${pal.darkBorder} bg-white px-5 py-3 text-sm font-bold ${pal.panelText} outline-none`}>
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="rounded-lg bg-white px-6 py-3 text-sm font-black text-[#0f4f59]">Filter</button>
          </form>
        </div>
      </section>
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-lg border ${pal.border} bg-white p-6 shadow-[0_20px_60px_rgba(15,79,89,0.08)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            ...
            <h1 className={`mt-4 max-w-4xl text-4xl font-black leading-tight tracking-[-0.04em] ${pal.panelText} sm:text-5xl`}>{post?.title || pagesContent.detailPages.article.fallbackTitle}</h1>
          </div>
          <aside className={`min-w-0 rounded-lg ${pal.darkBg} p-6 text-white`}>
      ...
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-lg border ${pal.border} bg-white p-6 shadow-[0_20px_60px_rgba(15,79,89,0.08)] sm:p-8 lg:p-10`}>
