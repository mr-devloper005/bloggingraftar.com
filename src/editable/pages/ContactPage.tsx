...
function getTone(kind: ReturnType<typeof getProductKind>) {
  ...
  if (kind === 'editorial') {
    return {
      shell: 'bg-[#f3f8f7] text-[#102a2f]',
      panel: 'border border-[#cddfdd] bg-white',
      soft: 'border border-[#d9e7e5] bg-white',
      muted: 'text-[#496266]',
      action: 'bg-[#0f4f59] text-white hover:bg-[#0b4149]',
    }
  }
  if (kind === 'visual') {
    return {
      ...
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8">
        <section className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-black leading-tight tracking-[-0.04em] sm:text-5xl">{pagesContent.contact.title}</h1>
          ...
          <div className={`rounded-lg p-7 ${tone.panel}`}>
